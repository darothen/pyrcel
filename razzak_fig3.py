...
mode2_kappas = np.linspace(0.01, 0.5, 6)
...
for kappa in mode2_kappas:
    ...
    for species in aer_species:
        ...
        lbl_kn, lbl_eq = ("Kinetic", "Equilibrium") if kappa == mode2_kappas[0] else (None, None)
        ax.plot(kappa, kn_frac, color="r", marker="D", linestyle="None", label=lbl_kn)
        ax.plot(kappa, eq_frac, color="b", marker="D", linestyle="None", label=lbl_eq)
    print " done"

kappas = np.linspace(0.01, 0.5, 100)
actfracs = []
for kappa in kappas:
    aerosol2 = AerosolSpecies('Mode 2', Lognorm(mu=0.05, sigma=2.0, N=100.), bins=200, kappa=kappa)
    initial_aerosols = [aerosol1, aerosol2, ]
