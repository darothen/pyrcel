...
def sigma_w(T):
    """Surface tension of water, J/m^2. See :func:`pyrcel.thermo.sigma_w`."""
    return 0.0761 - 1.55e-4 * (T - 273.15)


def dv_cont(T, P):
    """Continuum diffusivity of water vapor in air, m^2/s.

    See :func:`pyrcel.thermo.dv_cont`. ``P`` is in Pa.
    """
...
def dv(T, r, P, accom=c.ac):
    """Non-continuum-corrected diffusivity of water vapor, m^2/s.

    See :func:`pyrcel.thermo.dv`.
    """
    dv_t = dv_cont(T, P)
...
def es(T_c):
    """Saturation vapor pressure over water, Pa. ``T_c`` in **Celsius**.

    See :func:`pyrcel.thermo.es`.
    """
    return 611.2 * jnp.exp(17.67 * T_c / (T_c + 243.5))


def ka_cont(T):
    """Continuum thermal conductivity of air, J/m/s/K. See :func:`pyrcel.thermo.ka_cont`."""
    return 1e-3 * (4.39 + 0.071 * T)


def ka(T, rho, r):
    """Non-continuum-corrected thermal conductivity of air, J/m/s/K.

    Signature mirrors :func:`pyrcel.thermo.ka` (``T, rho, r``).
    """
    ka_t = ka_cont(T)
...
def rho_air(T, P, RH=1.0):
    """Density of moist air, kg/m^3. See :func:`pyrcel.thermo.rho_air`."""
...
def Seq_approx(r, r_dry, T, kappa):
    """Approximate κ-Köhler equilibrium supersaturation. See :func:`pyrcel.thermo.Seq_approx`."""
    A = (2.0 * c.Mw * sigma_w(T)) / (c.R * T * c.rho_w * r)
