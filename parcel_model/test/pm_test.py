...
solver_dt = 10. # simulation seconds; how frequently should the solver be re-set?
...
model = pm.ParcelModel(initial_aerosols, V, T0, S0, P0,
                       console=True, accom=accom)
#raw_input("Continue? ")
start = time.time()
parcel, aerosols = model.run(t_end, output_dt, solver_dt, # note new argument order!
                             terminate=True, terminate_depth=50.,
