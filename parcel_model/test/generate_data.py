...
def generate_reference(overwrite=False):
    ...
    print("rho_air", end=", ")
    results['rho_air'] = \
        [ thermo.rho_air(T, P*100) for T, P in \
            product(results['temperatures'], results['pressures']) ]
    print("(%d cases)" % len(results['rho_air']))

    print("es", end=", ")
    results['es'] = \
        [ thermo.es(T - 273.15) for T in results['temperatures'] ]
    print("(%d cases)" % len(results['es']))

    print("ka_cont", end=", ")
    results['ka_cont'] = \
        [ thermo.ka_cont(T) for T in results['temperatures'] ]
    print("(%d cases)" % len(results['ka_cont']))

    print("ka", end=", ")
    results['ka'] = \
        [ thermo.ka(T, rho, r*1e-6) for T, rho, r in \
          product(results['temperatures'], results['densities'],
                  results['radii']) ]
    print("(%d cases)" % len(results['ka']))

    print("sigma_w", end=", ")
    results['sigma_w'] = \
        [ thermo.sigma_w(T) for T in results['temperatures'] ]
